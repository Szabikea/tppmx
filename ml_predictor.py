...
class MLPredictor:
    ...
    def predict(self, 
        ...
        """
        # Lazy loading: csak az első kérésnél tanítunk
        if ML_AVAILABLE and not self.is_trained:
            print("[ML] Training models continuously on first request...")
            self.train()
...
def get_ml_predictor() -> MLPredictor:
    """ML predictor singleton, LAZY training"""
    global _ml_predictor
    if _ml_predictor is None:
        _ml_predictor = MLPredictor()
        # NINCS automatikus training importkor, csak kéréskor!
